...
| --- | ---------------------- | ---------------------------------------- | ------------------ | ---------------------------------------- | ---------------------------------------------- |
| 1   | Endpoint & Key         | `/endpoint-keys`                         | ada                | §7.3, §7.5 live                          | F5 **CLOSED**, F6 **CLOSED**, F7 **CLOSED**, F8 **CLOSED**, F9 **CLOSED**; draft 020 F1-F3 **CLOSED 2026-09-23** (kontrol copy di dalam dialog modal melaporkan sukses sementara clipboard kosong) |
...
| 4   | Usage                  | `/usage`                                 | ada                | §7.12 live                               | F8 **CLOSED**; F12; draft 014 F1-F6 **DURING selesai**, F7 OPEN; draft 015 F1-F6 **CLOSED 2026-09-22**; draft 016 F1-F4 **CLOSED 2026-09-22**, F5-F7 OPEN (permintaan `app-serv`), F8 **CLOSED 2026-09-23 oleh draft 018 F1** (tabrakan node 390px diperbaiki dan diukur ulang di browser); draft 018 F1-F4 **CLOSED 2026-09-23**; draft 022 F1-F2 **CLOSED 2026-09-23** (halaman tidak lagi bisa digeser ke area kosong di bawah isinya, dan teks statis di gambar node dihapus); draft 023 F1-F2 **CLOSED 2026-09-23** (paragraf penjelas di gambar diganti satu baris fakta di atas gambar mengikuti aturan warna gambar itu sendiri, dan daftar finished dipisahkan idiom tab tanpa satu pun elemen yang bisa difokus) |
...
- Tautan §6.5 di empty state `/usage` ("plus a link to API Docs") terpasang menyusul di pass
  "Usage parity" (draft 014 F5, 2026-09-22), jadi syarat di kalimat Risiko di atas sudah
  terpenuhi dan tidak lagi menunggu apa pun.
...
Pengukuran yang sama setelah pass "Usage parity" (2026-09-22) menemukan **dua belas dari dua puluh dua
bagian**, dan perintah yang sama mencetak `12 of 22` pada commit pass itu (`12 of 21` pada tree
sebelumnya). Penyebutnya naik satu sementara pembilangnya tetap: bagian ke-22 membawa click-through
browser yang direkam (dua belas tangkapan layar dan dua puluh nilai terukur di `evidence.json`), dan
bagian itu tidak menyebut click-through mana pun sebagai outstanding. Batasnya tetap dicatat di bagian
...
**dua belas dari dua puluh empat bagian**, dan perintah yang sama mencetak `12 of 24` pada commit pass itu
(`12 of 23` pada tree sebelumnya). Penyebutnya naik satu sementara pembilangnya tetap: bagian ke-24 membawa
click-through browser yang direkam di atas **route nyata** (lima tangkapan layar, tujuh kelompok nilai
terukur, plus 21 pemeriksaan klien), jadi tidak ada click-through yang disebut outstanding di bagian itu.
Batas yang masih terbuka di sana bukan lagi separuh gateway melainkan dua hal lain: satu cacat gambar di
viewport 390px yang ditemukan click-through itu dan dicatat sebagai F8 draft 016 (bukan outstanding, karena
click-through-nya sudah dijalankan dan cacatnya terukur), dan tiga selisih kontrak yang menunggu `app-serv`
(F5 sampai F7 draft 016: periode `All`, dimensi `Account`, drawer empat body).

Pengukuran yang sama setelah pass "Usage drawing sizing" (2026-09-23) menemukan **dua belas dari dua puluh
lima bagian**, dan perintah yang sama mencetak `12 of 25` pada commit pass itu (`12 of 24` pada tree
sebelumnya). Penyebutnya naik satu sementara pembilangnya tetap: bagian ke-25 membawa click-through browser
yang direkam di atas **route nyata** lagi (sepuluh tangkapan layar, delapan belas kelompok nilai terukur),
jadi tidak ada click-through yang disebut outstanding di bagian itu. Yang ditutup bagian itu adalah cacat
yang paragraf sebelumnya catat sebagai masih terbuka: F8 draft 016 diperbaiki (gambar mengambil ukurannya
dari lebar kotaknya) dan diukur ulang di browser pada 390px, dan cacatnya tidak lagi disebut terbuka di
bagian itu. Yang tetap terbuka: F5 sampai F7 draft 016 dan F13 di bawah.
...
Pengukuran yang sama setelah pass "The copy control inside a dialog" (2026-09-23) menemukan **dua belas dari
dua puluh tujuh bagian**, dan perintah yang sama mencetak `12 of 27` pada tree pass itu (`12 of 26` pada tree
sebelumnya). Penyebutnya naik satu sementara pembilangnya tetap: bagian ke-27 membawa click-through browser
yang direkam di atas **route nyata** (dua tangkapan layar, dua permukaan: modal kunci sekali tampil di
`/endpoint-keys` dan dialog `API base` di header, masing-masing menempelkan kembali nilai yang ditampilkan
layar), jadi tidak ada click-through yang disebut outstanding di bagian itu. Yang ditutup bagian itu adalah
satu cacat terukur di halaman Endpoint & Key: kontrol copy di dalam dialog modal melaporkan `Copied.`
sementara clipboard tetap kosong, karena scratch field jalur seleksi di-append ke `document.body` yang
inert saat `showModal()` terbuka, dan `execCommand` tetap menjawab true di sana (draft 020). Yang tetap
terbuka: F5 sampai F7 draft 016 dan F13 di bawah.

Pengukuran yang sama setelah pass "The usage page's scroll and the drawing's idle text" (2026-09-23)
menemukan **dua belas dari dua puluh delapan bagian**, dan perintah yang sama mencetak `12 of 28` pada tree
pass itu (`12 of 27` pada tree sebelumnya). Penyebutnya naik satu sementara pembilangnya tetap: bagian ke-28
membawa pengukuran browser yang direkam (enam keadaan halaman terukur pada dua build panel, dua tangkapan
layar, dan satu survei enam belas route) dan tidak menyebut click-through mana pun sebagai outstanding, jadi
bagian itu tidak terhitung. Yang ditutup bagian itu adalah satu cacat terukur yang owner laporkan di
`/usage`: dokumen bisa digeser ke area kosong di bawah isinya, karena setiap label `sr-only` di dalam
satu-satunya kolom penggeser berposisi terhadap halaman alih-alih terhadap kolom itu, sehingga labelnya
keluar dari potongan penggeser dan menambah area geser dokumen (591 px pada `/usage` dengan data, 8638 px
pada `/api-docs`, 822 px pada `/token-saver`, semuanya 0 sesudah kolom itu menjadi containing block). Koreksi
kedua owner di bagian yang sama juga ditutup di sana: teks statis di gambar node (daftar provider dan dua
kalimat diam) dihapus, hanya fakta yang sedang terjadi yang tersisa, dengan §6.5 diamandemen sejalan (draft
022). Batas yang dicatat di bagian itu: cabang kalimat live tidak dipotret di browser pada pass ini, dan
tidak ada test regresi jsdom untuk cacat geser itu karena jsdom tidak punya layout. Yang tetap terbuka: F5
sampai F7 draft 016 dan F13 di bawah.

Pengukuran yang sama setelah pass "The drawing's text and the finished list's separator" (2026-09-23)
menemukan **dua belas dari dua puluh sembilan bagian**, dan perintah yang sama mencetak `12 of 29` pada tree
pass itu (`12 of 28` pada tree sebelumnya). Penyebutnya naik satu sementara pembilangnya tetap: bagian ke-29
membawa lima pengukuran browser yang direkam (tiga tangkapan layar, satu probe stream diam pada halaman yang
sama) dan tidak menyebut click-through mana pun sebagai outstanding, jadi bagian itu tidak terhitung. Yang
ditutup bagian itu adalah dua item owner di `/usage`: paragraf penjelas di gambar node (warna dan beam) hilang
karena reference-nya tidak menggambar legenda, digantikan satu baris fakta di atas gambar dengan aturan warna
gambar itu sendiri, dan daftar finished dipisahkan idiom tab (label dengan rule aksen, tanpa `role`, 0 elemen
yang bisa difokus) setelah owner memilih pemisah statis alih-alih tab yang bisa diklik (draft 023). Premis
item kedua diukur lebih dulu dan tidak seperti yang dinyatakan: `/usage` milik reference punya dua tab
(Overview, Details) dan kartu finished-nya duduk di dalam Overview, bukan di tab sendiri. Batas yang dicatat:
warna `ok` pada nama provider yang sedang routing dibuktikan pada frame contract double, karena route gateway
...
tetap (`app-ui/scripts/boot-log.ts:35` mencetak `${Bun.version}`; sitasi ini sempat menunjuk baris 15,
yang berisi pembacaan `package.json`, dan dikoreksi saat pass ke-24 menyentuh bagian ini), jadi banner tidak mungkin
...
surface"), ke-21 ("Node motion"), ke-22 ("Usage parity"), ke-23 ("Node motion parity"), ke-24 ("Usage
sections parity and the real live route"), ke-25 ("Usage drawing sizing", 2026-09-23), ke-26
("Provider surface parity", 2026-09-23), ke-27 ("The copy control inside a dialog", 2026-09-23), dan
ke-28 ("The usage page's scroll and the drawing's idle text", 2026-09-23),
kesebelasnya
sudah memakai versi terukur, `1.3.0`, dan menunjuk temuan ini. Diukur ulang 2026-09-23 pada README yang
memuat bagian ke-26, dengan perintahnya: `grep -c '^Run with Bun 1\.3\.14' app-ui/README.md` mencetak
`16`, dan tiga baris tabel (Requirements, Node adapter, `bun run start`) melengkapinya menjadi sembilan
belas baris klaim, jadi jumlah klaimnya tidak berubah oleh pass ini; yang bertambah satu adalah baris
penunjuk, dari delapan menjadi sembilan (`grep -c '^Run with Bun 1\.3\.0' app-ui/README.md`). Diukur
ulang lagi pada README yang memuat bagian ke-27: perintah pertama tetap mencetak `16` (jumlah klaimnya
tetap sembilan belas), dan yang bertambah satu lagi adalah baris penunjuk, dari sembilan menjadi sepuluh.
Diukur ulang lagi pada README yang memuat bagian ke-28: perintah pertama tetap mencetak `16` (jumlah
...
klaim baru yang sama tidak terbuktinya.
