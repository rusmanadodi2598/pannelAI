...
|                      |                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Status**           | **CLOSED 2026-09-24, ditutup penuh 2026-09-25**: F1 dan F2 **CLOSED** di panel; F3 (filter aktif sisi server) dilayani `app-serv` dan sisi panel **ditutup oleh keputusan owner: tidak dipindah** (§3.3); F4 non-finding. Gerbang dan click-through terekam: §4                                                                                                                                                                                              |
...
**Status: CLOSED 2026-09-25 oleh keputusan owner; sisi `app-serv` dilayani, sisi panel sengaja tidak
dipindah.**

**Keputusan owner (2026-09-25, ralat atas usulan pindah picker):** "Jangan ubah `OpenCode`, serta
filtering provider/model aktif saat pick up Combo karena sudah benar seperti saat ini." Filtering
klien di `activeProviderIds` (`endpoint_count > 0 || no_auth`, amendemen F8 draft 029 untuk lorong
bebas-kredensial yang router layani lewat endpoint virtual) adalah aturan yang benar dan tetap
seperti adanya; picker Combo dan Vision tidak dipindah ke `?active=true`, dan permukaan `OpenCode`
tidak disentuh.

**Konteks terukur saat keputusan (2026-09-25, gateway hidup):** parameter sisi server ada dan
sehat (`?active=true` menjawab 9 baris dari 937; `?active=yes` 400), tetapi himpunannya bukan
padanan satu-satu dari aturan panel: jawaban membawa `mimo-free` dan `mmf` yang tidak ada di
daftar `GET /providers` (entri registry tersembunyi, yang panel buang menurut aturan
terdokumentasi), menyertakan `opencode` (`no_auth`, tanpa endpoint, dilayani endpoint virtual),
dan tidak membawa `edge-tts`, `google-tts`, `local-device`, `searxng` (semuanya `no_auth` dengan
baris katalog media). Memindahkan picker ke parameter itu menuntut penggabungan ulang di klien
untuk kasus-kasus itu, jadi dua bacaan akan tetap ada tanpa menyederhanakan apa pun; aturan "aktif"
yang benar bagi panel justru yang sekarang.

**Fakta (sebelum).** Panel membaca dua rute lalu menggabungkannya di klien: `GET /models/catalog` (587 baris,
...
**CLOSED 2026-09-24, ditutup penuh 2026-09-25.** F1 dan F2 ditutup di panel dengan test masing-masing;
F3: sisi `app-serv` dilayani (draft `025-CATALOG-ACTIVE-FILTER`, CLOSED) dan sisi panel ditutup oleh
keputusan owner tanpa pekerjaan (§3.3); F4 non-finding; F5 dan F6 ditutup setelah review gerbang pass
ini sendiri (§3.5, §3.6).
