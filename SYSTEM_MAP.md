...
| config | `internal/config`, `internal/registry`, `internal/provider` | Konfigurasi env bertipe, registry provider yang di-embed (termasuk tabel harga $/1M-token untuk estimasi cost chat, di-generate dari reference via `tools/pricing-gen.mjs` dan dipin corpus jawaban reference sendiri), dan seam plugin konektivitas per provider | memuat logika use case atau SQL |
