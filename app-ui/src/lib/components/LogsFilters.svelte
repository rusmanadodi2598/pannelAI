<script lang="ts">
	// The Logs filter bar (docs/SPEC-UI/001-SPEC-UI.md §6.11, tab 1).
	//
	...
	import { logFiltersApplied, type LogSearch } from '$lib/schemas/log-search';
	...
	type Props = {
		...
		onclear: () => void;
		/** Opens the purge dialog. The control lives here because it acts on what the filters select. */
		onpurge: () => void;
	};

	let { search, onchange, onapply, onclear, onpurge }: Props = $props();
...
<div class="flex flex-wrap items-end gap-3">
	...
	<button
		type="button"
		class="ml-auto min-h-11 rounded-[var(--radius-sm)] border border-[var(--color-danger)] px-3 text-sm text-[var(--color-danger)]"
		onclick={onpurge}>Purge captured logs</button
	>
</div>
