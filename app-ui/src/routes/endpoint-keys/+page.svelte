<script lang="ts">
	// Gateway keys tab of Endpoint & Key (docs/SPEC-UI/001-SPEC-UI.md §6.2).
	//
	// This screen owns the list, paging, and the two confirmation dialogs. Creating, rendering a row,
	// and showing the one-time key each live in their own component, which keeps every file under the
	// project line limit. Upstream endpoints and their multi-key list arrive in U1.
...
<section class="flex flex-col gap-5">
	<div class="flex flex-col gap-1">
		...
		<p class="text-sm text-[var(--color-text-muted)]">
			Keys a CLI tool presents to the gateway. Upstream endpoints and their keys arrive in the next
			phase.
		</p>
	</div>
